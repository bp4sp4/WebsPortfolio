...
export const project: ProjectEntry = {
  ...
  type: "company",
  title: "에듀바이저스",
  headline: "보상 구조를 한 흐름으로 전달하는 GA 리크루팅 사이트",
  ...
  detail: {
    ...
    changes: [
      {
        key: "정보 전달",
        before: "보상 구조와 지원 혜택을 상담을 통해서만 설명",
        after: "수익 구조 → 일반 GA 비교 → 혜택 → FAQ → 문의 CTA를 **한 흐름**으로 구성",
      },
      {
        key: "소개서",
        before: "소개서를 메일로 보내고 열어봤는지 알 수 없음",
        after: "리드 저장 후 열람 링크 발송, **열람 시각·횟수 기록**",
      },
      {
        key: "문의 대응",
        before: "문의가 메일함에 쌓여 대응이 늦어짐",
        after: "파트너 문의·교육 상담·채용 지원을 **Slack 즉시 알림**, 교육 상담은 한평생오피스 문의 DB로 자동 연결",
      },
      {
        key: "운영",
        before: "메인 팝업을 바꿀 때마다 개발자에게 요청",
        after: "어드민에서 팝업 **이미지·순서·활성 여부**를 직접 관리",
      },
    ],
    // diff 아래 숫자 한 줄
    metrics: [
      { value: "3", label: "문의 유형 → Slack 알림", icon: "fab fa-slack" },
      { value: "오피스", label: "교육 상담 → 문의 DB 자동 연결", icon: "fas fa-link" },
      { value: "SMTP", label: "소개서 발송 · 열람 추적", icon: "fas fa-envelope-open" },
      { value: "Figma", label: "디자이너 협업", icon: "fas fa-paint-brush" },
    ],
    outcome: [
