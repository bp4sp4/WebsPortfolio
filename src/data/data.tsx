...
export const projectDetails: { [key: string]: ProjectDetailType } = {
  ...
  solution: 
    "서버사이드에서 데이터를 안정적으로 처리하도록 설계하고, 클라이언트에서는 현재 어떤 정보가 입력되고 있는지 실시간 상태를 화면에 노출했습니다. 이를 통해 사용자는 업로드 과정을 직관적으로 파악할 수 있게 되었고, 대량 데이터 처리의 심리적 대기 시간을 줄였습니다"
  },
   {
  title: "DB Role 기반의 세분화된 직급 권한 체계 구축",
  challenge: 
    " 관리자, 매니저, 영업사원 등 사용자 역할에 따라 접근 권한과 노출되는 데이터의 범위가 엄격히 제한되어야 했습니다. 특히 정산과 관련된 민감한 정보의 노출을 막는 것이 관건이었습니다.",
  solution: 
    "데이터베이스의 Role 필드를 활용하여 직급별 권한을 정의하고, 이를 기반으로 페이지 접근을 제어하는 보안 가직을 구현했습니다. 이를 통해 인사 팀은 모든 정산 데이터를, 영업사원은 본인 담당 데이터만 볼 수 있도록 관리 체계를 명확히 했습니다."
  },
  {
  ...
  solution: 
    "PDF.js 라이브러리를 활용해 어떤 브라우저에서도 동일하게 문서가 보이도록 구현했습니다. 또한 지연 로딩(Lazy-loading) 방식을 적용하여, 사용자가 실제 문서를 클릭해 확인하는 시점에만 리소스를 불러오도록 설정해 전체 시스템의 초기 부하를 줄였습니다."
  },
    ],
    outcome: [
